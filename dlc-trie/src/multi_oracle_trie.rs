...
use crate::{DlcTrie, LookupResult, OracleNumericInfo, RangeInfo, TrieIterInfo};
...
impl MultiOracleTrie {
    ...
    pub fn dump(&self) -> MultiOracleTrieDump {
        MultiOracleTrieDump {
            ...
            extra_cover_trie_dump: match &self.extra_cover_trie {
                Some(trie) => Some(trie.dump()),
                None => None,
            },
        }
    }
    ...
    pub fn from_dump(dump: MultiOracleTrieDump) -> MultiOracleTrie {
        ...
        MultiOracleTrie {
            ...
            threshold,
            oracle_numeric_infos,
            extra_cover_trie: match extra_cover_trie_dump {
                Some(trie_dump) => Some(MultiTrie::from_dump(trie_dump)),
                None => None,
            },
        }
    }
    ...
    pub fn look_up(
        &self,
        paths: &[(usize, Vec<usize>)],
    ) -> Option<(RangeInfo, Vec<(usize, Vec<usize>)>)> {
    ...
    pub fn new(oracle_numeric_infos: &OracleNumericInfo, threshold: usize) -> Result<Self, Error> {
        if oracle_numeric_infos.nb_digits.len() < 1 {
...
impl<'a> DlcTrie<'a, MultiOracleTrieIter<'a>> for MultiOracleTrie {
    fn generate(
        ...
        if let Some(extra_cover_trie) = &mut self.extra_cover_trie {
            // Need to back-track on the CET index to get the last possible value.
            cet_index -= 1;
    ...
    fn iter(&'a self) -> MultiOracleTrieIter {
        ...
        let extra_cover_trie_iterator = match &self.extra_cover_trie {
            Some(extra_cover_trie) => Some(MultiTrieIterator::new(extra_cover_trie)),
            None => None,
        };
        MultiOracleTrieIter {
            digit_trie_iterator,
